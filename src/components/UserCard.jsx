  ...
  return (
    <div className="p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md">
      <div className="text-center">
        ...
        />
        <p className="font-semibold mt-4">{user.full_name}</p>
        {user.username && (
      ...
      <div className="flex mt-4 gap-2">
        <button
          ...
          disabled={currentUser?.followings.includes(user._id)}
          onClick={handleFollow}
        >
          <UserPlus className="w-4 h-4" />
          {currentUser?.followings.includes(user._id) ? 'Following' : 'Follow'}
        </button>
        <button
